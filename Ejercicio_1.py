...
def ejercicio_uno(sc,filename):
    ...
    adyacents = graph_clean.groupByKey() #RDD de tuplas donde a cada nodo le acompaña su RDD de nodos adjacentes, forma ('A', ['B','C','D','F']) 
    rdd_conexiones = adyacents.mapValues(list).flatMap(conexiones)  #RDD con todas las conexiones presentes en el grafo de la forma ((A, B) , exists) o bien ((B, C), ('pending', A))                   
